<script>
	...
	const sortedCourses = $derived(Object.fromEntries(
		Object.entries(courses).sort(([a], [b]) => a.localeCompare(b))
	));
	const filteredCourses = $derived(Object.fromEntries(
		Object.entries(sortedCourses).filter(([_,details])=>{const matchesType =
		details.type.toLowerCase() === type.toLowerCase()
		return matchesType}))
	);
</script>
