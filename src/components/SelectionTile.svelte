<script>
	let { fall, courses, year, courseSelected = $bindable() } = $props();
	...
	{#if courses}
		<div class="mx-4 flex flex-col gap-6">
			{#each courses as _, i (i)}
				<button
					class="font-noto-serif cursor-pointer rounded-full text-left text-xl ring-slate-900 transition-all duration-150 hover:-m-2 hover:bg-white hover:p-2 hover:ring-2"
					onclick={() => {
						courseSelected = `selection.${year}.${semester}[${i}]`;
					}}>{courses[i]}</button
				>
			{/each}
		</div>
